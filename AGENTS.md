...
src/
  app/                     # Next.js App Router
    (landing)/             # Rutas públicas (landing, contact, partners, how-it-works)
  ...
  styles/                  # Variables CSS y globals
```
...
- Animaciones sutiles con Framer Motion (no excesivas).

### 2.4 Performance
- Usar React Query para cache y data fetching.
- Componentes de servidor (Server Components) por defecto en Next.js.
- Usar `'use client'` solo cuando sea estrictamente necesario (interactividad, hooks).
...
- **MSW (Mock Service Worker)** para mocks de API en tests de integración.

### Estructura de Tests
```
__tests__/
├── components/          # Tests de componentes UI
├── hooks/               # Tests de TanStack Query hooks
└── e2e/                 # Flujos end-to-end (Playwright)
```

### Patrones de Testing
- **Componentes UI**: Render + assertions de contenido + interacciones.
- **Hooks TanStack Query**: Usar `QueryClientProvider` wrapper + MSW.
- **Flujos E2E**: Playwright para flujos completos (login → dashboard → acción).
...
❌ NO usar componentes UI sin shadcn/ui como base
...
- **shadcn/ui** como base para todos los componentes interactivos.
...
*[04/05/2026] Creación inicial del proyecto lalora-web - Estructura base con Next.js 15, Tailwind CSS 4, shadcn/ui. Landing page + panel de administración. Multi-tenancy con concepto de Company.*
